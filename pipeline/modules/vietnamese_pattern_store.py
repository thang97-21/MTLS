"""
Vietnamese Grammar Pattern Vector Store Module
Specialized wrapper for Vietnamese translation natural phrasing

This module provides:
1. Pre-built index for Japanese→Vietnamese grammar patterns
2. Pattern category filtering (contrastive_comparison, emotional_nuance, etc.)
3. Register awareness (casual/formal/literary)
4. Priority-based pattern injection for natural Vietnamese
5. Negative anchor system to suppress false-positive matches
6. Anti-AI-ism post-processing integration
"""
...
class VietnamesePatternStore:
    """
    Specialized vector store for Vietnamese natural phrasing patterns.

    Wraps PatternVectorStore with VN-specific:
    - Category filtering (contrastive_comparison, emotional_nuance, anti_ai_ism_patterns, etc.)
    - Register awareness (casual → formal → literary)
    - Japanese indicator matching
    - Priority-based pattern injection
    - Negative anchor penalty system
    - Anti-AI-ism detection hooks

    Architecture mirrors EnglishPatternStore but with Vietnamese-specific thresholds
    and additional anti-AI-ism post-processing.
    """

    # Vietnamese-specific thresholds (tuned for light novel genre)
    # Slightly lower than EN (0.78/0.65) because VN+JP bilingual embeddings
    # produce slightly lower cosine similarities in Gemini Embedding 001
    THRESHOLD_INJECT = 0.74  # High confidence: inject into prompt
    THRESHOLD_LOG = 0.62     # Medium confidence: log but don't inject

    # Negative Anchor thresholds
    # Identical to EN pipeline — proven effective in testing
    ...
    # Tuned for JP→VN light novel translation
    CATEGORY_PRIORITIES = {
        # Critical: these categories most affect VN naturalness
        "emotional_nuance": 10,           # やっぱり/まあ/さすが — highest impact on VN quality
        "anti_ai_ism_patterns": 10,       # một cách/một cảm giác — eliminate on sight
        "sentence_endings": 9,            # nhỉ/đấy/mà/nhé — Vietnamese particle system
        "comedic_timing": 9,              # Beat timing, tsukkomi — preserve humor
        "contrastive_comparison": 8,      # nhưng/mà/lại — common contrast patterns
        "action_emphasis": 8,             # đã...rồi/đang/mất — aspect markers
        "hedging": 8,                     # có lẽ/chắc/hẳn là — certainty gradation
        "keigo_register": 8,              # ạ/dạ/vâng — politeness system

        # High: important but less frequent
        "response_particles": 7,          # Ừ/À/Ồ — conversational markers
        "giving_receiving": 7,            # くれる/もらう social dynamics
        "structure_particles": 7,         # わけ/はず meaning shifts
        "modal_nuances": 7,              # nên/phải/bắt buộc — obligation scale
        "inner_monologue": 7,            # bất giác/chợt — narrator voice

        # Medium: supporting patterns
        "natural_transitions": 6,         # nên/nhưng mà/thôi — discourse connectors
        "concession_contrast": 6,         # dù/mặc dù/thế mà — despite-type
        "desire_intention": 6,            # muốn/thích/tò mò — wanting gradations
        "onomatopoeia": 6,               # hồi hộp/nhếch mép — mimetics
        "quotation_hearsay": 5,           # nghe nói/gọi là — indirect speech
        "time_expressions": 5,            # vừa/đã/rồi — temporal markers
        "intensifiers": 5                 # cực kỳ/siêu/quá — degree modifiers
    }

    def __init__(
        self,
        persist_directory: str = "./chroma_vietnamese_patterns",
        api_key: Optional[str] = None,
        rag_file_path: Optional[str] = None
    ):
        """
        Initialize Vietnamese pattern store.

        Args:
            persist_directory: ChromaDB storage location
            api_key: Gemini API key (uses GOOGLE_API_KEY env var if not provided)
            rag_file_path: Path to vietnamese_grammar_rag_v2.json
        """
        ...
        if rag_file_path is None:
            self.rag_file_path = Path(__file__).parent.parent / "config" / "vietnamese_grammar_rag_v2.json"
        else:
            self.rag_file_path = Path(rag_file_path)

        # Initialize core vector store with VN-specific collection
        self.vector_store = PatternVectorStore(
            collection_name="vietnamese_grammar_patterns",
        ...
        logger.info(f"VietnamesePatternStore initialized with persist_directory={persist_directory}")
        ...
        if collection_count == 0 and self.rag_file_path.exists():
            logger.warning("[VN-GRAMMAR] Vector store is empty — auto-rebuilding from vietnamese_grammar_rag_v2.json...")
            try:
                indexed = self.build_index(force_rebuild=False)
                total = sum(indexed.values())
                logger.info(f"[VN-GRAMMAR] ✓ Auto-rebuild complete: {total} patterns across {len(indexed)} categories")
            except Exception as e:
                logger.error(f"[VN-GRAMMAR] Auto-rebuild failed: {e}")
                logger.warning("[VN-GRAMMAR] Grammar pattern guidance will be unavailable this session")
        elif collection_count > 0:
            logger.info(f"[VN-GRAMMAR] Vector store loaded: {collection_count} indexed patterns")
    ...
    def load_rag_data(self) -> Dict:
        ...
        logger.info(f"Loaded VN RAG data from {self.rag_file_path}")
        return self._rag_cache

    def build_index(self, force_rebuild: bool = False) -> Dict[str, int]:
        """
        Build vector index from vietnamese_grammar_rag_v2.json.

        This extracts patterns from all categories and indexes them
        with appropriate metadata for Vietnamese natural phrasing.
        ...
        """
        if force_rebuild:
            logger.warning("Force rebuilding VN index - clearing existing data")
        ...
        for category_name, category_data in pattern_categories.items():
            ...
            logger.info(f"Indexing VN category: {category_name} ({len(patterns)} patterns)")
        ...
        logger.info(f"VN index built: {total_indexed} patterns across {len(indexed_counts)} categories")
    ...
    def _index_pattern(
        ...
        """
        Index a single pattern from vietnamese_grammar_rag_v2.json.

        Pattern structure:
        {
            "id": "vn_yappari_confirmed",
            "japanese_structure": "やっぱり / やはり / やっぱ",
            "japanese_indicators": ["やっぱり", "やはり", "やっぱ"],
            "vietnamese_pattern": "Context-dependent: đúng thật / thôi / quả nhiên",
            "examples": [
                {
                    "jp": "やっぱりあの子かわいいなぁ",
                    "literal": "Như mong đợi cô bạn đó dễ thương",
                    "natural": "Cô bạn đó dễ thương thật đấy…",
                    "source": "0b24_VN_corpus"
                }
            ],
            "usage_rules": [...],
            "priority": "critical"
        ...
        - Document: japanese_structure + japanese_indicators + jp example + natural VN
        - Metadata: vietnamese_pattern, natural, priority, category, usage_rules
        ...
        vietnamese_pattern = pattern.get("vietnamese_pattern", "")
        usage_rules = pattern.get("usage_rules", [])
        priority = pattern.get("priority", "medium")
        examples = pattern.get("examples", [])
        corpus_frequency = pattern.get("corpus_frequency", 0)
        ...
        for ex_idx, example in enumerate(examples):
            ...
            # VN-specific format: Japanese pattern + indicators + example + Natural VN
            # Using VN output in embedding allows better semantic matching
            text_parts = []
            ...
            # Add actual JP example (most important for context)
            if jp_text:
                text_parts.append(f"Example: {jp_text}")

            # Add natural Vietnamese for semantic similarity
            # (different from EN store which uses English)
            if natural:
                text_parts.append(f"Natural VN: {natural}")
            ...
            metadata = {
                ...
                "vietnamese_pattern": vietnamese_pattern,
                "jp_example": jp_text,
                "literal": literal,
                "natural": natural,
                "priority": priority,
                ...
                "source": source,
                "corpus_frequency": str(corpus_frequency)
            }
    ...
    def _build_negative_anchor_cache(self) -> Dict[str, List[np.ndarray]]:
        ...
        try:
            ...
            logger.warning(f"[VN-GRAMMAR] Failed to embed negative anchors: {e}")
            self._negative_anchor_cache = {}
            return self._negative_anchor_cache
        ...
        logger.info(f"[VN-GRAMMAR] ✓ Negative anchor cache built: {len(cache)} categories, {len(all_texts)} vectors")
        return self._negative_anchor_cache
    ...
    def _compute_negative_penalty(
        ...
        if max_neg_similarity >= self.NEGATIVE_ANCHOR_THRESHOLD:
            ...
            logger.debug(
                f"[VN-NEG-ANCHOR] {category}: neg_sim={max_neg_similarity:.3f} → penalty={penalty:.3f}"
            )
            return penalty
    ...
    def get_bulk_guidance(
        ...
        """
        Get Vietnamese pattern guidance for multiple detected Japanese patterns.
        ...
        for idx, (embedding, meta) in enumerate(zip(query_embeddings, query_metadata)):
            ...
            if results['ids'] and results['ids'][0]:
                for i in range(len(results['ids'][0])):
                    ...
                    # Build guidance entry (VN-specific: uses vietnamese_pattern instead of english_pattern)
                    guidance_entry = {
                        "pattern_id": results['ids'][0][i],
                        "similarity": similarity,
                        ...
                        "vietnamese_pattern": metadata.get("vietnamese_pattern", ""),
        ...
        # Sort by priority value then similarity (descending)
    ...
    def get_collection_stats(self) -> Dict:
        ...
        return {
            ...
            "negative_anchor": {
                "threshold": self.NEGATIVE_ANCHOR_THRESHOLD,
                "penalty": self.NEGATIVE_ANCHOR_PENALTY
            },
...
def quick_pattern_search(
    japanese_text: str,
    store_path: str = "./chroma_vietnamese_patterns"
) -> Dict:
    """
    ...
    Returns:
        Best match with Vietnamese pattern

    Example:
        result = quick_pattern_search("やっぱりあの子かわいいなぁ")
        # Returns: {"vietnamese_pattern": "đúng thật / thôi / quả nhiên", ...}
    """
    store = VietnamesePatternStore(persist_directory=store_path)

    results = store.search(query=japanese_text, top_k=1)

    if results and results[0]['similarity'] >= VietnamesePatternStore.THRESHOLD_INJECT:
        match = results[0]
        return {
            "japanese_text": japanese_text,
            "vietnamese_pattern": match['metadata'].get('vietnamese_pattern', ''),
...
if __name__ == "__main__":
    # Quick test
    print("Vietnamese Pattern Store Module loaded successfully")
    print(f"Thresholds: inject={VietnamesePatternStore.THRESHOLD_INJECT}, "
          f"log={VietnamesePatternStore.THRESHOLD_LOG}")
    print(f"Negative anchors: threshold={VietnamesePatternStore.NEGATIVE_ANCHOR_THRESHOLD}, "
          f"penalty={VietnamesePatternStore.NEGATIVE_ANCHOR_PENALTY}")
    print(f"Category priorities: {len(VietnamesePatternStore.CATEGORY_PRIORITIES)} categories configured")
